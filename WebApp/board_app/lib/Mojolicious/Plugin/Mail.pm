...
sub register {
    ...
    $app->helper(
        mail => sub {
            ...
            if ( $conf->{'how'} eq 'smtps' ) {
                my $mailer = new Net::SMTPS( @{ $conf->{'howargs'} || [] } )
                  or die "Failed to open SMTP connection: $!";
                $mailer->auth( $conf->{'AuthUser'}, $conf->{'AuthPass'} );
                $mailer->mail( $conf->{'from'} );
                $mailer->to( $args->{mail}->{To} );
                $mailer->data;
                ...
                $msg->send( $conf->{'how'}, @{ $conf->{'howargs'} || [] } )
                  unless $test;
                $msg->as_string;
            }
        },
...
sub build {
            ...
            $mail->{$_} = join ", ", grep {
                _enc( $_ => $charset );
                {
                    ...
                    $_ =
                      $name =~ /^[\w\s"'.,]+$/
                      ? "$name <$email>"
                      : MIME::EncWords::encode_mimeword( $name, $encode,
                        $charset )
                      . " <$email>";
                }
                1;
    ...
    $msg->add(%$_)
      for @{ $p->{headers} || [] };  # XXX: add From|To|Cc|Bcc => ... (mimeword)
    $msg->add(
        'X-Mailer' => join ' ',
        'Mojolicious', $Mojolicious::VERSION, __PACKAGE__, $VERSION, '(Perl)'
...
sub _enc($$) {
    ...
    $_[0] = b( $_[0] )->encode('UTF-8')->to_string
      if $_[0] && $charset && $charset =~ /utf-8/i;
    $_[0];
}
